...
function TaskItem(task, editable){
    this.conn = null;
    this._init(task, editable);
}

TaskItem.prototype = {
    // inherit from a PopupBaseMenuItem
    __proto__ : PopupMenu.PopupBaseMenuItem.prototype,
    _init: function(task, editable){
        PopupMenu.PopupBaseMenuItem.prototype._init.call(this);
        this.task = task;

        debug("\tGot task with name: "+ task.name);
    ...
    _clicked : function(actor, ev){
        ...
        if (ev.get_click_count() == 2 && this.task.file != null) {
            // On double click for project tasks, open the TODO
